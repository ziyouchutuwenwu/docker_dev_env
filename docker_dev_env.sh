...
docker run --rm -d --name php \
  -p 8888:80 \
  -v ~/projects/docker/web_root:/app \
...
docker run --rm -d --name nginx \
  -p 7777:7777 \
  -v ~/projects/docker/web_root:/web_root \
  -v $CURRENT_DIR"/conf/nginx/nginx.conf":/etc/nginx/nginx.conf \
...
docker run --rm -d --name mysql \
  -p 4407:3306 \
  -v ~/projects/docker/mysql:/var/lib/mysql \
  -e MYSQL_ROOT_PASSWORD=root \
  -e MYSQL_ALLOW_EMPTY_PASSWORD=true \
  --privileged=true \
  mysql:8.0

docker run --rm -d --name pgsql \
  -p 6543:5432 \
  -v ~/projects/docker/pgsql:/var/lib/postgresql/data \
...
docker run --rm -d --name ftp \
  -v ~/projects/docker/ftp/:/home/vsftpd/demo \
  -p 20:20 \
  -p 21:21 \
...
docker run --rm -d -it --name tftpd \
  -p 69:69/udp \
  -v ~/projects/docker/tftp:/srv/tftp \
  hkarhani/tftpd

# 共享目录为 \\ip\smb
docker run --rm -d --name smb \
  -v ~/projects/docker/smb/:/mount \
  -p 139:139 \
  -p 445:445 \
  dperson/samba \
